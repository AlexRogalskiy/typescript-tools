...
//     }
//
//     if (str.endsWith('ll') && gt1.test(str)) {
//         str = str.slice(0, -1)
//     }
//
...
//  */
// const cleanStopWords = (stopWords, words) => words.filter(tkn => !stopWords.includes(tkn))
//
...
//     deduplicateTokens(cleanServerStopWords(tokenize(str)).map(tkn => stem(tkn))).filter(
//         tkn =>
//             !!tkn &&
//             tkn.length > 1 &&
//             !/^-?\d+$/i.test(tkn) &&
...
//     deduplicateTokens(cleanClientStopWords(tokenize(str)).map(tkn => stem(tkn))).filter(
//         tkn =>
//             !!tkn &&
//             tkn.length > 1 &&
//             !/^-?\d+$/i.test(tkn) &&
