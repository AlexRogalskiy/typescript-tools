export function pluralize(entry: { zero: string; one: string; other: string }, count: number): string {
    let selectedEntry
...
export function pluralizeComments(count: number): string {
    return pluralize({ zero: 'No comments', one: '{count} comment', other: '{count} comments' }, count)
}
