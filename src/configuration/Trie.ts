import { TrieData, TrieLeafValue, TrieValue } from '../../typings/domain-types'

export class Trie {
    constructor(private root: TrieData) {}

    private _getLongestMatchingPrefix(key: string): any {
        ...
        while (node) {
            ...
            if (nextNode) {
                ...
                const matchedRegex: { matchedNode: TrieValue; matchLength: number } | undefined = Array.from(
                    node.keys(),
                )
                    .map(k => {
    ...
    get(key: string): string | string[] | undefined {
        ...
        const { remainingKey, value } = searchResult

        return remainingKey.length === 0 ? value : undefined
    }
    ...
    private static isLeafValue(value: any): value is TrieLeafValue {
