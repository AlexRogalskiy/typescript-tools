...
export const getFormattedDateTime = (date: string, prefix?: string, timeFormat = 'HH:mm, dd MMM'): string => {
    ...
    return converted > yesterday
        ? `${formatDistanceToNowStrict(converted)} ago`
        : `${prefix ? `${prefix} ` : ''}${format(new Date(converted), timeFormat)}`
}
