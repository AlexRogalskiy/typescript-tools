 ...
 */
export const isStable = (s: { x: number; v: number }, target: number, dt = 1 / 60): boolean =>
    Math.abs(s.x - target) < epsilon && Math.abs(s.v * dt) < epsilon

export const isStableAndBound = (s: { x: number; v: number }, target: number, dt?: number): boolean => {
