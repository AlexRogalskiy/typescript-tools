...
export function rgb([r, g, b, a]: number[]) {
    if (typeof a === 'number') {
        return `rgba(${r}, ${g}, ${b}, ${a})`;
    }
    return `rgb(${r}, ${g}, ${b})`;
...
export function mix(color1: number[], color2: number[], t: number) {
    return color1.map((c, i) => Math.round(c * (1 - t) + color2[i] * t));
}

export function toSVGMatrix(matrix: number[][]) {
    return matrix.slice(0, 4).map((m) => m.map((m) => m.toFixed(3)).join(' ')).join(' ');
}

export const rgbCacheKeys: Array<keyof RGBA> = ['r', 'g', 'b', 'a'];
export const themeCacheKeys: Array<keyof Theme> = ['mode', 'brightness', 'contrast', 'grayscale', 'sepia', 'darkSchemeBackgroundColor', 'darkSchemeTextColor', 'lightSchemeBackgroundColor', 'lightSchemeTextColor'];
