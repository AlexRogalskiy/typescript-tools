...
import { Checkers, Errors, Maths } from '..'
...
export namespace Sorting {
    ...
    export const bubbleSort = <T>(array: T[], cmp?: Comparator<T>): T[] => {
        if (!Checkers.isArray(array)) {
            throw Errors.valueError(`incorrect input parameter: array < ${array} >`)
        }
    ...
    export const bucketSort = (array: number[], cmp: Comparator<number>): number[] => {
        if (!Checkers.isArray(array)) {
            throw Errors.valueError(`incorrect input parameter: not array < ${array} >`)
        }
    ...
    export const hooraSort = (() => {
        ...
        return <T>(array: T[], left: number, right: number, cmp?: Comparator<T>) => {
            if (!Checkers.isArray(array)) {
                throw Errors.valueError(`incorrect input parameters: array < ${array} >`)
            }
    ...
    export const quicksortOptimized = (() => {
        ...
        return <T>(array: T[], cmp?: Comparator<T>): void => {
            if (!Checkers.isArray(array)) {
                throw Errors.valueError(`incorrect input array: < ${array} >`)
            }
    ...
    //
    //     if (min > max) {
    //         throw Errors.valueError(`incorrect min or max value: min < ${min} >, max < ${max} >`)
    //     }
    //
    ...
    export const combSort = (() => {
        ...
        return <T>(array: T[], cmp?: Comparator<T>): void => {
            if (!Checkers.isArray(array)) {
                throw Errors.typeError(`incorrect input argument: not array < ${array} >`)
            }
