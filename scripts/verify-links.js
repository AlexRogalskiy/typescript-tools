...
async function verifyUrl(basePath, absUrl, docPages) {
    ...
    if (
        absUrl.match(/https:\/\/github.com\/AlexRogalskiy\/typescript-tools\/(tree|blob)\/master\/docs\//) &&
    ...
    const url = absUrl
        .replace(/#.*$/, '')
        .replace(/https:\/\/github.com\/backstage\/backstage\/(tree|blob)\/master/, '')
    if (!url) {
        return undefined
    }
...
async function findExternalDocsLinks(dir) {
    ...
    for (const file of mdFiles) {
        ...
        const realPath = (match ? joinPath(dirname(url), match[1]) : url.replace(/\.md$/, '')).replace(
            /\/index$/,
            '',
        )
    ...
    if (badUrls.length) {
        ...
        for (const { url, basePath, problem } of badUrls) {
            if (problem === 'missing') {
                ...
                console.error('Links in docs must use absolute URLs for targets outside of docs')
                ...
                const suggestion =
                    docPages.get(url) || docPages.get(new URL(url, 'http://localhost').pathname)
                ...
                console.error(`Link to docs/ should not use a GitHub URL, use a relative URL instead`)
                console.error(`  From: ${basePath}`)
                console.error(`  To: ${url}`)
            }
        }
